    ...
    *phVadHandle = NULL;
    ...
    if ( pSpeechFlag ) {
        ...
        AUD_ASSERT( ret == 0 );
                ...
                pVadState->isNeedFlag = AUD_FALSE;
                pVadState->backTrackFlag.ringSize = backTrackDepth;
                pVadState->backTrackFlag.bufSize  = VAD_FRAME_LEN;
                ret = createBufRing( &(pVadState->backTrackFlag) );
                AUD_ASSERT( ret == 0 );
    }
...
AUD_Int32s sig_framecheck( void *hVadHandle, AUD_Int16s *pFrameBuf, AUD_Int16s *pFlag )
{
    ...
    logEngergyStd = sqrt( pVadState->logEnergyDev );
    ...
    if ( logEngergyStd < log10( pVadState->logEnergyAve  * 10000. ) / 900. )
    {
        logEngergyStd = log10( pVadState->logEnergyAve  * 10000. ) / 900.;
    }

    // AUDLOG( "s: %.5f, log_energy_ave: %.5f, log_energy_std: %.5f\n", s, pVadState->logEnergyAve, logEngergyStd );
    if ( s >  pVadState->logEnergyAve + logEngergyStd * 3. )
    {
        return 1;
    }
    else
